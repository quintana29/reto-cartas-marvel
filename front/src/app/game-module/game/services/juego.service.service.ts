...
import { QuitarCarta } from '../models/quitar-carta';
...
export class JuegoService {
  ...
  listarJuegos(idJugadorPrincipal: string | null): Observable<JuegoModel[]> {
    return this.http.get<JuegoModel[]>(
      `http://localhost:8080/juego/listar/${idJugadorPrincipal}`
    );
  }
  ...
  crearRonda(body: any) {
    return this.http.post(`http://localhost:8080/juego/crear/ronda/`, {
      ...body,
    });
  }
  iniciarRonda(body: any) {
    return this.http.post(`http://localhost:8080/juego/ronda/iniciar/`, {
      ...body,
    });
  }
  ...
  obtenerTableroPorJuego(juegoId: any): Observable<TableroModel> {
    return this.http.get<TableroModel>(
      `http://localhost:8080/juego/${juegoId}`
    );
  }
  ...
  obtenerMazo(uid: any, idJuego: any) {
    return this.http.get(
      'http://localhost:8080/juego/mazo/' + uid + '/' + idJuego
    );
  }
  listarTodosLosJuegos(): Observable<JuegoModel[]> {
    return this.http.get<JuegoModel[]>('http://localhost:8080/juegos/');
  }
  quitarCarta(body: any) {
    return this.http.post('http://localhost:8080/juego/quitar/', body);
  }
}
